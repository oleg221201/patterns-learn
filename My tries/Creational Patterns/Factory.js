...
var NftProvider = /** @class */ (function () {
    function NftProvider() {
    }
    return NftProvider;
}());
var OpenseaAsset = /** @class */ (function () {
    function OpenseaAsset(assetData) {
        this.assetData = assetData;
    }
    OpenseaAsset.prototype.validateAsset = function () {
        var _a;
        var asset = this.assetData;
        return {
            ...
            imageUrl: asset.image_url ||
                asset.collection.large_image_url ||
                asset.asset_contract.image_url,
            imageOriginal: asset.image_original_url,
            animationUrl: asset.animation_url,
            animationOriginal: asset.animation_original_url,
            info: {
                name: (_a = asset.name) !== null && _a !== void 0 ? _a : asset.token_id,
                creator: asset.creator.name || asset.creator.address,
                released: new Date(asset.collection.created_date).toLocaleDateString(),
...
var MoralisAsset = /** @class */ (function () {
    function MoralisAsset(assetData) {
        this.assetData = assetData;
    }
    MoralisAsset.prototype.validateAsset = function () {
        var asset = this.assetData;
        ...
        return {
            ...
            info: {
                name: metadata.name,
                released: new Date(asset.last_token_uri_sync).toLocaleDateString(),
...
var OpenseaProvider = /** @class */ (function (_super) {
    __extends(OpenseaProvider, _super);
    function OpenseaProvider() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    OpenseaProvider.prototype.getAsset = function (contract, tokenId) {
        console.log("get http request to https://api.opensea.io/api/v1/asset/".concat(contract, "/").concat(tokenId));
        return new OpenseaAsset(openseaResponse);
    };
    OpenseaProvider.prototype.showAsset = function (contract, tokenId) {
        var asset = this.getAsset(contract, tokenId);
        return 'Opensea validated asset: ' + JSON.stringify(asset.validateAsset());
    };
    return OpenseaProvider;
}(NftProvider));
var MoralisProvider = /** @class */ (function (_super) {
    __extends(MoralisProvider, _super);
    function MoralisProvider() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    MoralisProvider.prototype.getAsset = function (contract, tokenId) {
        console.log("get http request to https://deep-index.moralis.io/api/v2/nft/".concat(contract, "/").concat(tokenId, "?chain=polygon&format=decimal"));
        console.log('and get metadata from asset.tokenUri');
        return new MoralisAsset(__assign(__assign({}, moralisResponse), { assetMetadata: moralisMetadataResponse }));
    };
    MoralisProvider.prototype.showAsset = function (contract, tokenId) {
        var asset = this.getAsset(contract, tokenId);
        return 'Moralis validated asset: ' + JSON.stringify(asset.validateAsset());
    };
    return MoralisProvider;
}(NftProvider));
function testCode(provider, contract, tokenId) {
    var asset = provider.showAsset(contract, tokenId);
    console.log(asset);
}
console.log('opensea process:');
testCode(new OpenseaProvider(), 'contract123', 'tokenId123');
console.log('------------------');
console.log('');
console.log('');
console.log('');
console.log('moralis process:');
testCode(new MoralisProvider(), 'contract456', 'tokenId456');
