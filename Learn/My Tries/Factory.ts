...
interface IAsset {
    validateAsset(): AssetType
}

abstract class NftProvider {
    public abstract getAsset(contract: string, tokenId: string): IAsset

    public abstract showAsset(contract: string, tokenId: string): string
}

class OpenseaAsset implements IAsset {
    ...
    public validateAsset(): AssetType {
...
class MoralisAsset implements IAsset {
    private assetData
...
class OpenseaProvider extends NftProvider {
    public getAsset(contract: string, tokenId: string): IAsset {
        console.log(`get http request to https://api.opensea.io/api/v1/asset/${contract}/${tokenId}`);
        return new OpenseaAsset(openseaResponse)
    }

    public showAsset(contract: string, tokenId: string) {
        const asset = this.getAsset(contract, tokenId)
        return 'Opensea validated asset: ' + JSON.stringify(asset.validateAsset());
    }
}

class MoralisProvider extends NftProvider {
    public getAsset(contract: string, tokenId: string): IAsset {
        console.log(`get http request to https://deep-index.moralis.io/api/v2/nft/${contract}/${tokenId}?chain=polygon&format=decimal`);
        console.log('and get metadata from asset.tokenUri');
        return new MoralisAsset({...moralisResponse, assetMetadata: moralisMetadataResponse});
    }
...
function testCode (provider: NftProvider, contract: string, tokenId: string) {
    const asset = provider.showAsset(contract, tokenId)
    console.log(asset);
}
